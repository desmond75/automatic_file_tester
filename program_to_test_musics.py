...
def automatic_file_tester(full_path_to_file,number_of_files_to_test, test_time_in_seconds):
	""" # full_path_to_file is where your file is located eg if your file is located in D drive 
	    ...
	    """
	for file in islice(os.listdir(full_path_to_file),number_of_files_to_test):
		#loops through the files in the path provided
		absolute_path_to_file = full_path_to_file + file #file name is added to the full_path_to_file
		time.sleep(test_time_in_seconds) #waiting time during each file testing
		os.startfile(absolute_path_to_file)#opens file using the default program on your computer
